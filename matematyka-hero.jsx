...
export default function App(){
  ...
  useEffect(()=>{(async()=>{try{const r=await window.storage.get("math-hero-v3");if(r?.value){let s=JSON.parse(r.value);
    ...
    setStats(s);try{await window.storage.set("math-hero-v3",JSON.stringify(s));}catch(e){}}
    const lg=await window.storage.get("math-hero-lang");if(lg?.value)setLang(lg.value);}catch(e){}})();},[]);
  const save=useCallback(async s=>{setStats(s);try{await window.storage.set("math-hero-v3",JSON.stringify(s));}catch(e){}},[]);
  const setLanguage=async(l)=>{setLang(l);try{await window.storage.set("math-hero-lang",l);}catch(e){}};
